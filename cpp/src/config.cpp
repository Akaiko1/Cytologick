...
fs::path findConfigInDir(const fs::path& dir) {
...
std::string defaultConfigYaml() {
    const Config defaults{};
    return
R"(# Autogenerated default config for Cytologick.
general:
  openslide_path: ""
  hdd_slides: "./current"

neural_network:
  image_shape: [)" + std::to_string(defaults.imageShape[0]) + ", " + std::to_string(defaults.imageShape[1]) + R"(]
  image_chunk: [)" + std::to_string(defaults.imageChunk[0]) + ", " + std::to_string(defaults.imageChunk[1]) + R"(]
  classes: )" + std::to_string(defaults.classes) + R"(
  batch_size: )" + std::to_string(defaults.batchSize) + R"(
  labels:
    LSIL: 2
    HSIL: 2
    "Group HSIL": 2
...
gui:
  ...
  default_threshold: )" + std::to_string(defaults.defaultThreshold) + R"(
)";
}

bool writeDefaultConfigFile(const fs::path& path) {
...
Config Config::load() {
    ...
    fs::path configPath = findConfigInDir(exeDir);

    if (configPath.empty()) {
        const fs::path generatedPath = exeDir / "config.yaml";
        if (writeDefaultConfigFile(generatedPath)) {
            std::cout << "Created default config: " << generatedPath << std::endl;
            configPath = generatedPath;
        } else {
            std::cerr << "Failed to create default config in executable directory: "
                      << generatedPath << std::endl;
        }
    }

    // Fallback for development runs if file creation in executable directory failed.
    if (configPath.empty()) {
        configPath = findConfigInDir(fs::current_path());
    }
